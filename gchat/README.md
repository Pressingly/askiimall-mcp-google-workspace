...
The `Service Account Email` field is auto-generated by Google in the form `service-<project_number>@gcp-sa-gsuiteaddons.iam.gserviceaccount.com` — no input needed.
...
Filter modes (`all`, `prefix`, `allowlist`, `denylist`, `category`) are implemented in `askiimall-mcpo/app/service_gateway/utils/tool.py`. Restart MCPO after changing the filter so the cached tool list rebuilds.
