...
async def create_doc(
    ...
    content: str = Field('', description="Optional initial plain-text content. For rich formatting (headings, bold, lists, tables), create the doc then use format_doc_paragraph, modify_doc_text, insert_doc_elements, and create_table_with_data."),
) -> str:
    """
    Creates a new Google Doc and optionally inserts initial plain-text content.

    Content is inserted verbatim as plain text (Markdown is not rendered). To build
    a formatted document, create it here, then apply structure with the granular
    tools: format_doc_paragraph (headings/alignment), modify_doc_text (bold/italic/
    links), insert_doc_elements (lists, tables, page breaks), create_table_with_data.
